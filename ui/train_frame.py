...
from config import App, LAYER_PARAMS
...
class TrainFrame(ctk.CTkFrame):
    ...
    def _add_layer(self):
        ...
        for param_name, param_info in param_template.items():
            default_value = param_info["default"]           # Get default value from parameter info
            default_params[param_name] = default_value      # Add it to default parameter dictionary
    ...
    def _refresh_layer_list(self):
        ...
        for index, layer in enumerate(self.layers):
            ...
            layer_item.bind("<Button-1>", lambda event, index=index: self._select_layer(index))
            layer_label.bind("<Button-1>", lambda event, index=index: self._select_layer(index))
        ...
        if index == None:
          ...
          msg.pack(pady=20)
            ...
            for param_name, param_info in param_template.items():
                ...
                param_label.grid(row=row_number, column=0, padx=10, pady=10, sticky="w")

                if param_info["type"] == "int":
                    param_entry = ctk.CTkEntry(self.param_panel)
                    param_entry.insert(0, str(param_value))
                    param_entry.grid(row=row_number, column=1, padx=10, pady=10, sticky="ew")

                    param_entry.bind("<Return>", lambda event, param_name=param_name: self._update_param(param_name, event.widget.get()))

                elif param_info["type"] == "dropdown":
                    param_combo = ctk.CTkComboBox(self.param_panel, values=param_info["options"])
                    param_combo.set(param_value)
                    param_combo.grid(row=row_number, column=1, padx=10, pady=10, sticky="ew")

                    param_combo.configure(command=lambda val, p=param_name: self._update_param(p, val))
    ...
    def setup_model_selection(self):
        ...
        self.model_selection_label.grid(
            row=self.current_row, column=0, padx=10, pady=10, sticky="w"
        )

        self.model_selection = ctk.CTkComboBox(self, values=App.MODEL_LIST)
        self.model_selection.grid(
            row=self.current_row, column=1, padx=10, pady=10, sticky="ew"
        )

        self.new_model_button = ctk.CTkButton(self, text="New Model")
        self.new_model_button.grid(
            row=self.current_row, column=2, padx=10, pady=10, sticky="w"
        )
    ...
    def _move_up(self):
        if self.selected_layer_index is not None and self.selected_layer_index > 0:
            self.layers[self.selected_layer_index], self.layers[self.selected_layer_index - 1] = self.layers[self.selected_layer_index - 1], self.layers[self.selected_layer_index]
    ...
    def setup_layer_editor(self):
        ...
        self.layer_selector = ctk.CTkComboBox(self, values=App.LAYER_LIST)
        self.layer_selector.grid(
            row=self.current_row, column=1, padx=10, pady=10, sticky="ew"
        )
        ...
        self.button_frame.grid(
            row=self.current_row, column=0, columnspan=2 ,padx=5, pady=0, sticky="ew"
        )
        ...
        self.movedown_button.pack(side="right")
    ...
    def setup_hyperparameters(self):
        ...
        self.epochs_entry = ctk.CTkEntry(self, placeholder_text="100")
        self.epochs_entry.grid(
            row=self.current_row, column=1, padx=10, pady=10, sticky="ew"
        )

        self.current_row += 1

        self.batch_size_label = ctk.CTkLabel(self, text="Batch Size")
        self.batch_size_label.grid(
            row=self.current_row, column=0, padx=10, pady=10, sticky="w"
        )

        self.batch_size_entry = ctk.CTkEntry(self, placeholder_text="32")
        ...
        self.optimizer_label = ctk.CTkLabel(self, text="Optimizer")
        self.optimizer_label.grid(
            row=self.current_row, column=0, padx=10, pady=10, sticky="w"
        )

        self.optimizer_entry = ctk.CTkComboBox(self, values=App.OPTIMIZER_LIST)
        self.optimizer_entry.grid(
            row=self.current_row, column=1, padx=10, pady=10, sticky="ew"
        )
    ...
    def setup_bottom_buttons(self):
        ...
        self.button_frame.grid(
            row=self.current_row, column=0, columnspan=5, padx=10, pady=10, sticky="e"
        )
        ...
        self.reset_button = ctk.CTkButton(self.button_frame, text="Reset Model")
        self.reset_button.pack(side="right", padx=10)

        self.save_button = ctk.CTkButton(self.button_frame, text="Save Model")
